...
uint256 hashGenesisBlock("0x2c890f2275e83be13bbd209cafa234204926454e6442ccf56cae8232ad6124f7");
...
const string strMessageMagic = "MarsBux Signed Message:\n";
...
{
    // MarsBux: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // MarsBux: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "mars genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MarsBuxMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MarsBuxMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MarsBuxMiner(CWallet *pwallet)
{
    printf("MarsBuxMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("MarsBux-miner");
    ...
    try { loop {
        ...
        printf("Running MarsBuxMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MarsBuxMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MarsBuxMiner, pwallet));
}
