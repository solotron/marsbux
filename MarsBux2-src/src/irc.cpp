...
// Copyright (c) 2011-2012 MarsBux2 Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #MarsBux2TEST0\r");
            Send(hSocket, "WHO #MarsBux2TEST0\r");
        } else {
            // randomly join #MarsBux200-#MarsBux299
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #MarsBux2%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #MarsBux2%02d\r", channel_number).c_str());
        }
