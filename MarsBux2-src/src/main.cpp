...
uint256 hashGenesisBlock("0xb753f85dd05f916c52bfb45bea632905555fb122dace5a2c1f75c64b24cc1442");
...
const string strMessageMagic = "MarsBux2 Signed Message:\n";
...
{
    // MarsBux2: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // MarsBux2: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "mars2 genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MarsBux2Miner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MarsBux2Miner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MarsBux2Miner(CWallet *pwallet)
{
    printf("MarsBux2Miner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("MarsBux2-miner");
    ...
    try { loop {
        ...
        printf("Running MarsBux2Miner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MarsBux2Miner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MarsBux2Miner, pwallet));
}
