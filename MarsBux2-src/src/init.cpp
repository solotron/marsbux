...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("MarsBux2 version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  MarsBux2d [options]                     " + "\n" +
                  "  MarsBux2d [options] <command> [params]  " + _("Send command to -server or MarsBux2d") + "\n" +
                  "  MarsBux2d [options] help                " + _("List commands") + "\n" +
                  "  MarsBux2d [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "MarsBux2:"))
                fCommandLine = true;
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: MarsBux2.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: MarsBux2d.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 33813 or testnet: 19663)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 33812 or testnet: 19666)") + "\n" +
        ...
        "\n" + _("SSL options: (see the MarsBux2 Wiki for SSL setup instructions)") + "\n" +
...
//    }
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. MarsBux2 is probably already running."), strDataDir.c_str()));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("MarsBux2 version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
        fprintf(stdout, "MarsBux2 server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of MarsBux2") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart MarsBux2 to complete") << "\n";
