...
// Copyright (c) 2011-2012 MarsBux Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #MarsBuxTEST0\r");
            Send(hSocket, "WHO #MarsBuxTEST0\r");
        } else {
            // randomly join #MarsBux00-#MarsBux99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #MarsBux%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #MarsBux%02d\r", channel_number).c_str());
        }
